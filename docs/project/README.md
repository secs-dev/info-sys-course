...
Обычно последние 2 этапа разработки back- и front-end частей системы выполняется
одновременно, но отчеты и защита все равно должны соответсвовать общим
...
возможность делать Review отдельных групп изменений (GitHub).

Практик должен иметь возможность проверить исходный код системы и сделать Review правок для отдельных этапов работы, оставлять комментарии к отдельным строчкам исходного кода.
...
> В отчете должна быть только существенная и доступная информация для целевого
  читателя этого отчета (практика).
...
- не должно быть кного сплошного текста,
- графические элементы (таблицы, схемы, диаграммы) предпочтительны для
  отображения сложных и важных зависимостей.
...
Оптимальный вариант языка разметки — [Typst][typ], т.к. он обладает упрощенным синтаксисом в сравнении с LaTeX и схож с привычным Markdown. Он позволяет разрабатывать документы, как в Online-сервисе, так и локально с использованием всего лишь одной утилиты без множества зависимостей. Более того, есть готовый [шаблон typst-документа в формате ГОСТ][typ-template].

### Работа с проектной документацией.

В рамках первого этапа работы составляется спецификация требований к системе. Она следует стандартному формату, например, SRS. **Спецификация** — это отдельный документ, отличный от **отчета** за этап.

В спецификации следует использовать различные средства визуализации для описания
предметной области и требований к системе, например, Use-Case диаграммы,
ER-диаграммы[^ER-slides][^ER-yndx][^ER-vid] (не путать с даталогическими для 2
этапа), BPMN-диаграммы, С4-cхемы и тд. Также рекомендуется начинать с простых
текстовых инструментов описания предметной области, таких как User Story,
которые позднее могут стать основанием для планирования работы с помощью User
Story Mapping и формирования подробных требований и пользовательских сценариев.
